...
public class FirstCourseQueryRepositoryImpl extends QuerydslRepositorySupport implements FirstCourseQueryRepository {
    ...
    @Override
    public List<AllCourseListResponse.FirstCourse> firstCourseListForDiy(AllCourseListRequest request) {
        QFileManagerEntity file = new QFileManagerEntity("file");
        QDirManagerEntity dir = new QDirManagerEntity("dir");
        ...
        return queryFactory.select(Projections.constructor(AllCourseListResponse.FirstCourse.class,
                ...
                .where(
                        firstCourseEntity.withdrawal.isFalse(),
                        firstCourseEntity.endDate.goe(Expressions.currentTimestamp()).or(firstCourseEntity.endDate.isNull()),
                        searchByTitle(request.getTitle()),
                        searchByMainAddress(request.getMainAddress()),
                        searchBySubAddress(request.getSubAddress())
                )
                .fetch();
    }

    private BooleanExpression searchByTitle(String title) {
        return title != null ? firstCourseEntity.firstCourseTitle.contains(title) : null;
    }

    private BooleanExpression searchByMainAddress(String mainAddress) {
        return mainAddress != null ? firstCourseEntity.address.contains(mainAddress) : null;
    }

    private BooleanExpression searchBySubAddress(String subAddress) {
        return subAddress != null ? firstCourseEntity.address.contains(subAddress) : null;
    }
