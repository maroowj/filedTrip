...
//        System.out.println(">>>>" + request.toString());
        ...
        if (request.getTerms1() != null) {
            parseTerms1 = (Map<String, Object>) jsonParser.parse(request.getTerms1());
        } else {
            parseTerms1 = null;
        }
        if (request.getTerms2() != null) {
            parseTerms2 = (Map<String, Object>) jsonParser.parse(request.getTerms2());
        } else {
            parseTerms2 = null;
        }
        if (request.getTerms3() != null) {
            parseTerms3 = (Map<String, Object>) jsonParser.parse(request.getTerms3());
        } else {
            parseTerms3 = null;
        }
        if (request.getTerms4() != null) {
            parseTerms4 = (Map<String, Object>) jsonParser.parse(request.getTerms4());
        } else {
            parseTerms4 = null;
        }
        if (request.getTerms5() != null) {
            parseTerms5 = (Map<String, Object>) jsonParser.parse(request.getTerms5());
        } else {
            parseTerms5 = null;
        }
        ...
        int timeCnt = 0;
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String requestTime = request.getReservationDate().substring(11, 19);
        ...
        // 로그인이 되어 있으면 memberSeq를 넣음
        Optional<MemberEntity> optionalMember = memberRepository.findById(SecurityContextHolder.getContext().getAuthentication().getName());
        ...
        if(member == null && password == null) {
            result = 3; // 비로그인 상태에서 비밀번호가 존재하지 않음
        }else {
            List<ReservationEntity> reservationList = reservationRepository.findAllByFirstCourseAndReservationDateAndBookingStatusNot(firstCourse,  sdf.parse(request.getReservationDate()), "예약취소");
            ...
            if (timeCnt <= reservationList.size()) {
                ...
                reservationRepository.save(
                        ReservationEntity.builder()
                                ...
                                .reservationDate(sdf.parse(request.getReservationDate()))
...
//                mailService.sendReservationMail(request);
    ...
    public Map<String, Object> reservationDetail(ReservationDetailRequest request) {
        ...
        if (member != null) {
            if (!reservationEntity.getMember().equals(member)) {
                data.put("result", "failed");
                data.put("message", "회원이 예약한 내역을 찾을 수 없습니다.");
            } else {
...
//                    System.out.println(">>" + secondCourse);
            ...
            if(reservationEntity.getMember()!=null) {
                data.put("result", "failed");
                data.put("message", "비회원으로 예약되었습니다. (로그아웃 이후 조회 가능)");
            }else {
                String password = request.getPassword();
                if(password==null) {
                    password="0000";
                }
                if (passwordEncoder.matches(password, reservationEntity.getPassword())) {
...
//                            System.out.println(">>>" + strCheckDate);
...
//                            }
    ...
    public int cancelReservation(ReservationDetailRequest request) {
        ...
        if (member != null) {
            if (!reservation.getMember().equals(member)) {
                result = 0; // 비밀 번호 불일치 또는 본인 예약건이 아닌 경우 (예약취소 실패)
            } else {
    ...
    @Transactional
    public Page<UserReservationListResponse> userReservationList(Pageable pageable) {
        Optional<MemberEntity> optionalMember = memberRepository.findById(SecurityContextHolder.getContext().getAuthentication().getName());
        MemberEntity member = null;
        if (!optionalMember.isPresent()) {
        ...
        return reservationRepository.userReservationList(pageable, member);
    }
}
